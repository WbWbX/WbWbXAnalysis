...
triggerAnalyzer::selectDileptons(std::vector<ztop::NTMuon> * inputMuons, std::vector<ztop::NTElectron> * inputElectrons){
	...
	for(size_t i=0;i<inputMuons->size();i++){
		...
		if(muon->pt() < DEF_MUON_PT)       continue;
		...
		if((muon->isGlobal() || muon->isTracker())
			//	&& muon->normChi2()<10.
			//	&& muon->muonHits()>0
	...
	//	if(suclueta > 1.4442 && suclueta < 1.5660) continue; //transistion region
		...
		//	idelectrons <<  elec;
			if(!noOverlap(elec,globalmuons,0.1))
				continue;
			if(fabs(elec->rhoIso())<0.15){
				//isoelectrons <<  elec;
				tempelecs << elec;
			}
