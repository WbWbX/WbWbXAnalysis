...
invokeApplication(){
	...
	//nMC = 1000;
	std::vector<ztop::NTJet> * pJetMC = 0;
	...
	for (int i = 0; i < nMC; i++) {
	 ...
	 //        if(jet->btag() > 0.244) haybjet++;
		...
		for(std::vector<ztop::NTElectron>::iterator Elec=pElecs2->begin(); Elec != pElecs2->end(); Elec++){
			if(Elec->mHits() > maxmHits || !Elec->isNotConv() || fabs(Elec->d0V()) >= d0Cut || (doRhoIso && Elec->rhoIso()>= IsoCut) || (!doRhoIso && Elec->isoVal() >= IsoCut ) || Elec->pt() < 30.0 || fabs(Elec->eta()) > EtaCut || !Elec->isPf() || Elec->storedId() <= mvaCut) continue;
			...
			//TRIGGER
			int foundProbe = 0;
			for(std::vector<ztop::NTElectron>::iterator Elec2=pElecs2->begin(); Elec2 < pElecs2->end(); Elec2++){
				if (foundProbe>0) break;
