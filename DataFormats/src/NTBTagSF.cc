...
void NTBTagSF::addInSampleNames(const TString& thisstr,const TString & with){
	std::cout << "NTBTagSF::replaceInSampleNames: Caution! should not be used under normal circustances" << std::endl;

	std::map<std::string,std::vector<TH2D> >::iterator histit;
	std::map<std::string,std::vector<TH2D> >::iterator effit=effhistos_.begin();
	std::map<std::string,std::vector<float> >::iterator medvecit=medianMap_.begin();
	for(histit=histos_.begin();histit!=histos_.end();++histit){
		TString oldname=histit->first;
		oldname.ReplaceAll(thisstr,with);
		histos_[oldname.Data()]=histit->second;
		effhistos_[oldname.Data()]=effit->second;
		medianMap_[oldname.Data()]=medvecit->second;
	//	histos_.erase(histit);
	//	effhistos_.erase(effit);
	//	medianMap_.erase(medvecit);
		effit++;
		medvecit++;
	}
	std::cout << "NTBTagSF::replaceInSampleNames: Sample names now:" <<std::endl;
	listAllSampleNames();
}
