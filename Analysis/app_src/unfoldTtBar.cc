...
void unfold(int argc, char* argv[]){
    ...
    bool ignoremass = parse.getOpt<bool>("i",false);
    TString signal= parse.getOpt<TString>("s","");
    bool moreoutput=parse.getOpt<bool>   ("v",false);
    std::vector<TString> allin = parse.getRest<TString>();
    if(allin.size()>1 || allin.size()<1){
        std::cout << "Must specify exactly one input file" << std::endl;
        return;
    }
    TString in=allin.at(0);
    if(!in.EndsWith(".root"))
        in.Append(".root");
    TString outfile=in;
    ...
    for(size_t i=0; i<csvsize;i++){
        ...
        if(stack.is1DUnfold()){
            ...
            if(use){
                if(signal!="") std::cout << "Set signal to " << signal << " for stack " << stack.getName() <<std::endl;
